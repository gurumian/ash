...
/**
 * Custom hook for managing SSH/Serial/Local connections
 */
export function useConnectionManagement({
  ...
  // Helper: Complete Local session setup after successful connection
  const completeLocalSessionSetup = useCallback(async (session, connection) => {
    // Initialize terminal if not already initialized
    if (!terminalInstances.current[session.id]) {
      setTimeout(() => {
        initializeTerminal(session.id, session);
  ...
  const createNewSessionWithData = useCallback(async (formData, skipFormReset = false) => {
    ...
    try {
      ...
      if (formData.connectionType === 'ssh') {
        ...
        sessionName = formData.sessionName || 'Local Terminal';
      } else {
      ...
      if (formData.connectionType === 'ssh') {
        ...
        if (useTelnet) {
          ...
          await new Promise((resolve) => {
            setTimeout(() => {
              initializeTerminal(sessionId, session).then(() => resolve()).catch(() => resolve());
            }, 0);
          });

          // Wait for terminal
          let attempts = 0;
          while (attempts < 20 && !terminalInstances.current[sessionId]) {
  ...
  const disconnectSession = useCallback(async (sessionId) => {
    ...
    if (fitAddons.current[sessionId]) delete fitAddons.current[sessionId];
    if (searchAddons.current[sessionId]) delete searchAddons.current[sessionId];

    await cleanupLog(sessionId);
    if (cleanupStopwatch) cleanupStopwatch(sessionId);
  ...
  const reconnectSSHSession = useCallback(async (session) => {
    if (sshConnections.current[session.id]) {
      ...
      } catch (error) { console.log('Error disconnecting old SSH connection:', error); }
    ...
    if (!retryEnabled) {
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        if (terminal) terminal.write(`\r\x1b[K\x1b[90mAttempting ${attempt}/${maxAttempts}...\x1b[0m`);
        try {
          connection = new SSHConnection();
          await connection.connect(session.host, session.port, session.user, password, session.name, session.id);
          sshConnections.current[session.id] = connection;
          if (terminal) terminal.write(`\r\x1b[K\x1b[90mConnected successfully.\x1b[0m\r\n`);
          break;
        } catch (error) {
          lastError = error;
          ...
          if (attempt < maxAttempts) {
            ...
            if (terminal) terminal.write(`\r\x1b[K\x1b[90mDisconnected. Max retry attempts (${maxAttempts}) reached.\x1b[0m\r\n`);
  ...
  const reconnectTelnetSession = useCallback(async (session) => {
    if (sshConnections.current[session.id]) {
      try {
        sshConnections.current[session.id].disconnect();
      } catch (error) { console.log('Error disconnecting old Telnet connection:', error); }
      delete sshConnections.current[session.id];
    }
  ...
  const reconnectSerialSession = useCallback(async (session) => {
    ...
    if (sshConnections.current[session.id]) {
      try {
        if (terminal) terminal.write('\r\n\x1b[90mResetting connection...\x1b[0m\r\n');
        sshConnections.current[session.id].disconnect();
      } catch (error) { console.log('Error disconnecting old Serial connection:', error); }
      delete sshConnections.current[session.id];
    }
    ...
    if (!terminalInstances.current[session.id]) {
      ...
      if (terminal) terminal.write('\x1b[90mConnected successfully.\x1b[0m\r\n');
      setTimeout(() => {
        executePostProcessing(connection, session.id);
      }, 300);
    }
  ...
  // Helper: Reconnect Local session
  const reconnectLocalSession = useCallback(async (session) => {
    const terminal = terminalInstances.current[session.id];
    ...
    if (terminal) {
      terminal.write('\r\n\x1b[90mLocal terminal restarted.\x1b[0m\r\n');
    }

    await completeLocalSessionSetup(session, connection);

  }, [terminalInstances, completeLocalSessionSetup]);
  ...
  const reconnectSession = useCallback(async (session) => {
    try {
      if (session.connectionType === 'ssh') {
        ...
        await reconnectLocalSession(session);
      }
    } catch (error) {
  ...
  }, [reconnectSSHSession, reconnectTelnetSession, reconnectSerialSession, reconnectLocalSession, formatConnectionError, setErrorDialog]);
  ...
  useEffect(() => {
    const handleRemoveDetachedSession = (sessionId) => {
      setSessions(prev => {
        ...
        if (cleanupTerminal) cleanupTerminal(sessionId);
        if (fitAddons.current[sessionId]) delete fitAddons.current[sessionId];
        if (searchAddons.current[sessionId]) delete searchAddons.current[sessionId];
          ...
          setTimeout(async () => {
            try {
              if (newSession.connectionType === 'ssh') {
                ...
                // Serial reconnect not fully implemented here for detach
                setSessions(prev => prev.map(s => s.id === newSession.id ? { ...s, isConnected: false } : s));
              } else if (newSession.connectionType === 'local') {
                // Local reconnect for detach
                const connection = new LocalConnection();
                await connection.connect(newSession.id);
                sshConnections.current[newSession.id] = connection;
                setSessions(prev => prev.map(s => s.id === newSession.id ? { ...s, isConnected: true } : s));
                setTimeout(() => {
                  initializeTerminalRef.current(newSession.id, newSession);
                }, 100);
              }
